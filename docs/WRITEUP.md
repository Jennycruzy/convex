...
*Session scope:* **1** kill switch (append-only file, every cycle) · **2** **calibration: no position is opened while any cost or liquidity input is still an unmeasured guess.** `config/convex.yaml` names its unmeasured values where the code reads them, not only where a reader sees them, and this check refuses the session until measurement against live quotes has replaced them. The per-contract fees ship at zero, and trading on a zero fee understates the net-of-cost hurdle in the direction that makes a candidate look tradeable when it is not · **3** market calendar, from Alpaca's own endpoint rather than a local holiday table · **4** daily loss limit, 3% of equity then halt and publish · **5** buying power, verified against the account · **6** cumulative cost budget, 2% of equity.
